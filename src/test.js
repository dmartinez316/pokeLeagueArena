...
const settings = { "MAX_CHALLENGES": 2 };
...
exports.chomposGen = function () {
    ...
    let testCases = {};
    let testCasesFormated = {};

    for (let index = 0; index < 5; index += 1) {
        const gD = generateDeltas(0.3);

        let minimumMovements = 0;
        let finalResults = [];

        for(var e = 0; e < gD.result.length; e += 1){ 
        //for (const element in gD.result){
           // console.log(element);
            finalResults.push(gD.result[e].name);
            minimumMovements += gD.result[e].challenges;
        }
        testCasesFormated[index] = {"results":finalResults,"minMovs":minimumMovements};
        testCases[index] = gD;
        //console.log(gD);
    ...
    var jsonContent = JSON.stringify(testCasesFormated);
